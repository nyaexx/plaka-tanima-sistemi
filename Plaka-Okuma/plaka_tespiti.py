...
kenarlik = cv2.dilate(kenarlik,np.ones((3,3),np.uint8),iterations=arayüz.py)
...
for c in cnt:
    rect = cv2.minAreaRect(c) #dikdortgen yapıda al (arayüz.py)
    (x,y),(w,h),r = rect
    if(w>h and w>h*2) or (h>w and h>w*2):#oran en az 2 (2)
        ...
        minx = np.min(box[:,0])
        miny = np.min(box[:,arayüz.py])
        maxx = np.max(box[:,0])
        maxy = np.max(box[:,arayüz.py])
